...
servlet的生命周期非常简单：只有一个主状态：初始化。一个servlet要么未被`init()`，要么正在被`init()`，要么正在被`destroy()`，或者还不存在。 其生命周期如下图所示：
...
如果有多个请求到来，每次容器都会创建一个新线程并调用servlet的`service()`方法。通常，有多少请求就有多少线程在运行。 servlet类不会有多个实例，但是**容器会运行多个线程来处理对一个servlet的多个请求**
。对应每个客户请求，都会生成一对新的请求和响应对象。

### 加载和初始化

容器启动后，它会自动寻找已部署的Web应用，并搜索Servlet类文件。此后容器会加载类，可能是容器启动时，或在客户首次使用到这个Servlet时，取决于容器的配置。但是，在`init()`方法完成前绝不能运行`service()`方法。

> 通常，`init()`总是在第一个`service()`调用前完成。

创建后的Servlet还只是一个普通对象而不是一个Servlet。它还需要一些东西来初始化。二者的关键区别在于Servlet持有两个特定的对象：
...
Servlet的请求和响应是通过相对应的对象来实现的。它们是`service()`方法的参数。

![Servlet请求和响应对象](resources/Servlet请求和响应对象.png)

相关的类的实现由开发商来负责的，它们的具体类型不用你关心，只用知道它们肯定实现了以上接口就行了。

### HTTP方法

客户的请求总是包括一个特定的HTTP方法，如果是GET, 那`service()`就会调用`doGet()`方法；如果是POST, 那就是`doPost()`方法。 HTTP 1.1
协议规定了8种不同的方法，每种Servlet都有对应的`doXXX()`实现，不过大部分情况下你只会用到GET和POST方法。
...
POST不同于GET的根本区别在于，POST有消息体。虽然它们都能发送参数，但GET的参数必须写在请求行上。因此导致了以下区别：
...
注意，如果你没有指定method的话，会默认调用GET方法。而你如果又没有在Servlet中覆盖`doGet()`方法的话……那可就是大麻烦了。如果你想要同时支持这两个方法，可以实现`doGet()`方法，然而在`doPost()`
中调用`doGet()`方法。
...
响应是为了写回信息。通常你使用响应对象得到输出流，并写回HTML或其他内容。为了实现目标：
...
注意：如果写入响应后，再调用重定向方法就会抛出`IllegalStateException`异常。

重定向和请求分派的区别：重定向是servlet完成，而请求分派是由容器完成。